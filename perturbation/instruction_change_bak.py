...
def disassemble_and_modify(filepath, output_filepath):
           ...
           ((section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_MEM_WRITE']) or 
            ...
            for instr in decoder:
                try:
                    ...
                    if 'add' == op:
                        if op_0 in reg_32 and op_1 in reg_32 or ('h' in op_1):
                            new_text+=  instruction
                            continue

                        new_ins = 'sub '+op_0+', -'+op_1
                        machine_code = assemble_asm(new_ins, KS_ARCH_X86, bit)
                        mc_code =  bytes.fromhex("".join("{:02x}".format(byte) for byte in machine_code)) #여기 16진수 함 고쳐보자 저거 다 
                        new_text += mc_code
                        continue
    ...
    #                             if op_1 not in reg_32 and section_start <= int(op_1.rstrip('h'), 16) <= section_end: #or ('FF' in op_1):
    #                                 new_text +=  instruction
    #                                 continue
                                ...
                                if len(mc_code) < len(instruction):
                                    ...
                                    while len(mc_code) < len(instruction):
                                        machine_code = assemble_asm('nop', KS_ARCH_X86, bit)
                                        nop_code =  bytes.fromhex("".join("{:02x}".format(byte) for byte in machine_code))
                                        mc_code+=b'\x90'

                                    #print("  -->",mc_code, len(mc_code) == len(instruction))
                                    #print("     -->",mc_code, instruction, len(mc_code),len(instruction),'\n')
                                    new_text += mc_code
                                    continue
...
def main():
    sample_dir = '../sample/labeling/'
    save_dir_base = '../sample/perturbated_labling_sample/instruction_change_new_5/'
