...
def modify_headers(file_path, new_text, fin=None):
    ...
    for section in pe.sections:
        if b'.text' in section.Name.strip(b'\x00').lower() or b'.clone' in section.Name.strip(b'\x00').lower():
...
def modify_tramp(save_dir, modified_address, fin=None):
    ...
    array_offset = 0
    new_value = None  # new_value를 기본값으로 설정
    ...
    for (offset, size, instruction, hexdump) in decoded_instructions:
        if '90' not in hexdump:
            ...
            new_address = modified_address.get(target_address)

            if new_address is None:
                print(f"Error: Target address {hex(target_address)} not found in modified_address.")
                continue
            ...
            new_value = bytes.fromhex(('e9' + operand))  # new_value를 할당
            break

    if new_value is None:
        print("Error: No valid instruction found to modify.")
...
def modify_rdata(save_dir, modified_address, fin = None):
    ...
    for section in pe.sections:
        ...
        if section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_CNT_INITIALIZED_DATA'] and \
            ...
            # 절대 주소 필터링을 위한 범위 설정
            image_base = pe.OPTIONAL_HEADER.ImageBase
            text_section = next(section for section in pe.sections if  b'.text' in section.Name.strip(b'\x00').lower() or b'.clone' in section.Name.strip(b'\x00').lower())
...
def make_new_text(file_path ,number_of_nop):
    ...
    for section in pe.sections:
        ...
        if (section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_CNT_CODE']) and (section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_MEM_READ']) and \
           ...
           (section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_CNT_INITIALIZED_DATA']))and (b'.text' in section.Name.strip(b'\x00').lower() or b'.clone' in section.Name.strip(b'\x00').lower()):
...
def valid_value_int_convert(value):
    value = value[0].replace('h','',1)
    value = value.replace(',','',1)
    hex_value = value.replace('[','',1).replace(']','',1)
...
def unmatched_address_chacker(modified_address, caller_callee_dict, checking_target_address):
    ...
    for caller, callee in checking_target_address.items():
        try:
            original_caller = new_address_to_ori_address[caller]
            original_callee = caller_callee_dict[original_caller]
            adding_nop_callee = modified_address[original_callee]

            if adding_nop_callee != callee:
                modifying_address[caller] = adding_nop_callee
...
def modify_instruction_at_address(pe_data, section, target_address, new_bytes, image_base):
    # 섹션의 파일 오프셋 계산 (섹션의 VirtualAddress에서 상대적인 주소를 계산)
    section_start = section.VirtualAddress + image_base
    section_offset = section.PointerToRawData

    # target_address에 해당하는 파일의 오프셋 계산
    relative_address = target_address - section_start
    file_offset = section_offset + relative_address

    # 수정할 위치에 새로운 명령어 (hex 코드) 삽입
    print(hex(file_offset), hex(file_offset + len(new_bytes)), new_bytes)
    pe_data[file_offset:file_offset + len(new_bytes)] = new_bytes
...
def is_direct_address(target, decoded_instructions):
    ...
    for (offset, size, instruction, hexdump) in decoded_instructions:
        if offset == target:
            target_address = immediate_pattern.findall(instruction)
            
            if target_address:
                target_address = target_address[0]#.replace('0x','0')
                int_value = int(target_address, 16)  # 16진수 문자열을 정수로 변환
                formatted_value = f"{int_value:08x}"  # 8자리로 맞추기, 앞에 0 채우기
                print(formatted_value)  # 출력: '0899a82b'
                print("target_address : ",formatted_value)
                target_address = to_little_endian(formatted_value)
                print("   target_address : ",target_address)
                print(target_address, type(target_address), instruction, type(instruction))
                
            else:
                #print("Tlqkffkal ?? : ",target_address, type(target_address), instruction, type(instruction))
                continue
            
            if (target_address in hexdump) and ('ff' not in hexdump):
                return "ABS", offset, instruction, hexdump, size, immediate_pattern.findall(instruction)
            else:
                return "REL", offset, instruction, hexdump, size, immediate_pattern.findall(instruction)
...
def valid_address_check(file_path, save_dir, caller_callee_dict, checking_target_address, modified_address, number_of_nop):
    ...
    for section in pe.sections:
        ...
        if (section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_CNT_CODE']) and (section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_MEM_READ']) and \
           ((section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_MEM_WRITE']) or 
            (section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_MEM_EXECUTE']) or \
           (section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_CNT_INITIALIZED_DATA']))and ('.text' in section_name or '.clone' in section_name):
            text_section = section
            ...
            if pe.FILE_HEADER.Machine == 0x8664:
                bitness = 64
                address_pattern_short = re.compile(r'\b0x[0-9A-Fa-f]{12}\b')
                address_pattern_long = re.compile(r'\b0x[0-9A-Fa-f]{16}\b')
            else:
                bitness = 32
                address_pattern_short = re.compile(r'\b0x[0-9A-Fa-f]{6}\b')
                address_pattern_long = re.compile(r'\b0x[0-9A-Fa-f]{8}\b')

            decoded_instructions = decode_instructions(binary_data, text_start, bitness)

            modifications = []  # 수정할 명령어와 주소를 저장할 리스트

            for target, destination in modifying_address.items():
                #print(hex(target), hex(destination))
                result = is_direct_address(target, decoded_instructions)
                
                if result is None:
                    continue
                else:
                    address_type, address, instruction, hexdump, size, immediate_pattern  = result
                    
                print(address_type, address, instruction, hexdump, type(hexdump), size, immediate_pattern)
                ...
                present_instr = present_instr.hex()

                if 'ABS' in address_type:
                    ...
                    if value:
                        value, hex_value = valid_value_int_convert(value)
                        hex_value = hex_value.replace('0x','',1)
                    ...
                    if text_start <= value <= text_end:
                        ...
                        operand = to_little_endian(operand)
                        #print("op hex : ",op_code, operand, type(op_code), type(operand))
                        new_ins = op_code + operand
                    ...
                    if value:
                        ...
                        if value:
                            new_ins, op_code, operand = calc_offset(destination, address, ori_operand, op_code, size)
                # 각 target 주소와 new_ins 명령어를 리스트에 저장
                print("   -->",new_ins)
                modifications.append((target, bytes.fromhex(new_ins)))

            # 여러 명령어를 한 번에 수정
            for target_address, new_bytes in modifications:
                pe_data = modify_instruction_at_address(pe_data, text_section, target_address, new_bytes, image_base)
                
            binary_data_list.append(pe_data)
...
def list_files_by_size(root):
    # 파일들을 크기순으로 정렬하여 반환
    files_with_size = []
    for file_name in os.listdir(root):
        file_path = os.path.join(root, file_name)
        if os.path.isfile(file_path):
            files_with_size.append((file_name, os.path.getsize(file_path)))
    # 파일 크기 순으로 정렬 (오름차순)
    files_with_size.sort(key=lambda x: x[1])
    return [file for file, size in files_with_size]

def process_sample(args):
    sample, root, save_dir, number_of_nop = args
...
def main():
    ...
    sample_list = ['de1b7bd2b8366a15f27b1e204e04b572c1c8e58bf1e98fb1aff5192951f56364.exe', 'c3d94ae9c921b67fdcb2d952a5072d77ca343f29040f9db0e9b834c027b78385.exe', '5918d607ae0c82cdc97cdfbc5c6010c65f43ef32fa36ff7bac86af1b5c14c954.exe', 'a2e85ec840b29b0f4160f249094639a05c2fa6ea5d672af35e5e403eafb10cb4.exe', 'c20e9208750f47a286b121d44d9d2bcde6f6386913df5eb79aa20bb2ea80dda3.exe', '4c2bfd20ab5ee66e47d10a5df518adf6a7c246dd2d7e1efdb1923761d2484669.exe', '8f7d0ae066c88b077fb3005c3b846a48818dd1dc7606bcc2f6702808df651c9b.exe', '13d9c96c57ad169e8cceb38a5dde6b328d21d291d485a61272f932cac10503e1.exe', 'fed6739d4efd79a19c379230fdbac805f722057ec48dbb397e9a8617ee0a94e8.exe', 'b5322ccef79fa7a07492e1b90c40d83d77731ec5aa3b6f8ac0b0f7618492da98.exe', '3fc339331da123bd2f79efee661fdca9a48388fda9798c82c7565c3a78e3822e.exe', '3e65f191440dbe024364e93dee963207646cdf230b59754df221fa8c542fce6b.exe', '40d9b777a449aa99eb1e7383dfa3f157f2a83c24103097ee75747196d36c107a.exe', '320943d1635533fcd4e1ad25aeaf127e8f004890d5bc30a207ed32f6884af53e.exe', '3d6a9abcdcef4c2d379be5f6825532784bf9676480b20f44d8e9deebeb43010e.exe', '82fc8a529093179ec01093714b424b40c345cb5a715f61817104dc3c4e684b59.exe', '8804614370899a14f2397014972a506553d0ec898240797d71070fd7daac0b75.exe', '8fec940ed876c179217a5a26efb497b59892dcea07a32da201035b5dce486def.exe', 'ef573b8a1a9ded83830af339ad0d536af07c5c65a4a27a1a207ad02979b1b197.exe', '4841ee4f19bca8e7be520c0430fd71ef3a7d0ff96195f0e5c564617cd33771b9.exe', 'ee93fa766506941a18c63aefd71af63cc5b5669f01a53091db4b15dd78326388.exe', '15a5e7452155069444548108da7855ffb5bd31dc2440f4a47de73f53d81eb23f.exe', '0f571ef05bcef1cc66e59e7db9fa4d0ae8a68a0eaf4efe9db207f6175638ea85.exe', 'd2a6a5ca03ff06f3ee86a7c076cbf1f3cd627eeb54aa2a33fb5f561eb3b42990.exe', 'a9b62632b8fa6fae60e79ab6aab3ac0df2d95828eca03ceb3d0ab6ecc2b6315d.exe', 'ef90e35e7c2be389293d12d9e5da1aaf5551d563d54b8aa546e9dd306f602fb1.exe', '87080bf25c45ac1d98a739632777c211763d9d4517dcfecbbfaa3b8694810003.exe', '6a344818a876a1440591ca477ecd9e147209938ea7884f221cf467a2dfa53aca.exe', '086a8ecdffdae7c11bf06db7be42507ddfa69bbc35b81dc998b168dc0b6b00e9.exe', '9d33d44c05170a89db0facde27cb3573be8a0be38039430a8789bf584f17eabb.exe', 'd35d5eb2f526f7c7db6d622027e8184665cc09980ca5b7a0e4eab69452353e06.exe', 'c21fd428ae8b740ef0ce2a73060e908b039df192cfc13dcedb4460411319488f.exe', '86e5e4a2a4125d6bab039678df13e741ab8531d939b498e445ba12d093613a60.exe', '9b5cf968e994bfd4bd2d0fa4e5fc65730f4c31a15b965428f90c63a1188b595c.exe', 'edd9bbc1382119e5cb00fc038e840eaf99b2a4632bb6d548ba19dfbdf05d4ca7.exe', 'fe72c2eae080212830e8ced2f3c771e4c09c109fb9004f00833cad893b782c39.exe']
    
    sample_dir = '../sample/perturbated_labling_sample/section_move_new/'
    save_dir_base = '../sample/perturbated_labling_sample/adding_nop_new/'
    
    tasks = []
    
    # 디렉토리 내 파일 개수를 기준으로 작은 디렉토리부터 처리
    for root, dirs, files in os.walk(sample_dir):
        ...
        # 파일 크기 순으로 정렬된 파일 리스트 반환 후 멀티프로세싱을 위한 작업 리스트 생성
        for sample in list_files_by_size(root):
            ...
            if sample in sample_list:
                ...
                tasks.append((sample, root, save_dir, 1))
    ...
    # 모든 작업이 완료된 후 가비지 컬렉션 최종 실행
    gc.collect()
    print("All tasks are completed and memory has been cleaned.")

if __name__ == '__main__':
    #address = '2ba89908'
    #print(to_little_endian(address))
    main()
