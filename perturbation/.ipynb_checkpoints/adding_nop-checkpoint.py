...
def modify_section(file_path, new_text, save_dir, number_of_nop, fin=None):
    ...
    #print("sibal : ",file_format, file_path)
    
    with open(tmp_file, "rb") as tmp:
        tmp_binary = tmp.read()
    
#     if '_tmp' in file_path:
#         file_path = file_path.split('_tmp',1)[0]
    
    new_binary = tmp_binary[:old_rawPointer] + new_text[0] + tmp_binary[old_rawPointer + len(new_text[0]):]
   # print("save_dir : ",save_dir,'|',file_path.split('/')[-1].replace(file_format, f"_nop_fin_{number_of_nop}{file_format}"))
    if fin:
        if '.exe' in save_dir:
            os.remove(save_dir)
            save_dir = os.path.join(*[part for part in save_dir.split(os.sep) if '.exe' not in part]) 
...
def valid_address_check(file_path, save_dir, caller_callee_dict, checking_target_address, modified_address, number_of_nop):
    ...
    print("valid_address_check:", len(modifying_address))
...
#             new_text = modify_headers(file_path, new_text)
#             save_path = modify_section(file_path, new_text, save_dir, number_of_nop)
#             print(len(modified_address))
#             modify_tramp(save_path, modified_address)
#             save_path = modify_rdata(save_path, modified_address)
#             valid_address_check(file_path, save_path, caller_callee_dict, checking_target_address, modified_address, str(number_of_nop))
#             print("Done!!", sample)
#             asdf()
            
#     except pefile.PEFormatError:
#         print("parsing error : ", sample)
#         pass
...
# #     with multiprocessing.Pool(processes=num_processes) as pool:
# #         results = [pool.apply_async(process_sample, args=(task,)) for task in tasks]
...
#     # Gather all directories and count the number of files in each directory
#     directories = [(root, count_files_in_directory(root)) for root, dirs, files in os.walk(sample_dir) if len(files) > 0 and 'ok' not in root.split(os.sep)]

#     # Sort directories by the number of files (smallest to largest)
#     directories.sort(key=lambda x: x[1])

#     # Iterate over the directories and prepare the tasks
#     for root, file_count in directories:
#         if file_count <= 10:
#             continue
...
#     sample_dir = '../sample/perturbated_labling_sample/section_move/'
#     save_dir = '../sample/perturbated_labling_sample/adding_nop/'
...
#         number_of_nop = 1
#         #print(list_files_by_size(root))
#         for sample in samples:
            
#             save_dir = '../sample/perturbated_labling_sample/adding_nop/'
#             if '.ipynb' in sample or '.pickle' in sample or '.txt' in sample or '.zip' in sample or '.' not in sample:
#                 continue
#             if '68db9d1a67602e20f056472d370c6a6d86dd9b276ab87c9c86b561f6cdf3ddc2.exe' not in sample:
#                 continue
            
#             fam = root.split('/')[-1]
# #             output_filename_1 = sample.replace('.exe', '_adding_'+str(number_of_nop)+'.exe')
# #             output_filename_2 = sample.replace('.exe', '_nop_fin_'+str(number_of_nop)+'.exe')
# #             output_filepath_1 = os.path.join(save_dir+fam, output_filename_1)
# #             output_filepath_2 = os.path.join(save_dir+fam, output_filename_2)

# #             # 두 가지 파일 중 하나라도 존재하면 작업을 건너뜀
# #             if os.path.isfile(output_filepath_1) or os.path.isfile(output_filepath_2):
# #                 continue
                
# #             if '7d74d5d594245f3f416d6de3552cedcc2f950981d86554f749338468cdbb9ebb.exe' not in sample:
# #                 continue
...
#                     #print("len_new_text: ",len(new_text))
#                     new_text = modify_headers(file_path, new_text)
#                     save_dir = modify_section(file_path, new_text, save_dir+root.split('/')[-1]+'/' ,number_of_nop)
#                     #print(len(modified_address))
#                     modify_tramp(save_dir, modified_address)
#                     save_dir = modify_rdata(save_dir, modified_address)
#                     valid_address_check(file_path, save_dir, caller_callee_dict, checking_target_address, modified_address, str(number_of_nop))
#                     print("Done!!",save_dir+sample)
                    
#             except pefile.PEFormatError:
#                 continue
                
#             finally:
#                 # 명시적으로 가비지 컬렉션을 호출하여 메모리 회수
#                 gc.collect()
#-------------------------------전체 디렉토리 다 훑어서 파일용량 작은순으로 진행--------------------------------
# def count_files_in_directory(directory):
#     # Count the number of files in the directory (excluding subdirectories)
#     return sum([len(files) for _, _, files in os.walk(directory)])
...
#     except KeyError:
#         print("sample : ",sample) # 8ac8ced37b0c236860fb856a6a243a274c9919e6bd17d7c8ec99c2997dad94e3.exe 
# #                                       File "adding_nop.py", line 145, in modify_tramp
# #                                         new_address = modified_address[target_address]
# #                                     KeyError: 4317728
...
# def gather_all_files(sample_dir):
#     # Gather all files from all directories
#     all_files = []
#     for root, dirs, files in os.walk(sample_dir):
#         if len(files) <= 50 or 'ok' not in root.split(os.sep):
#             for file in files:
#                 # Filter out unwanted file types
#                 if any(ext in file for ext in ['.ipynb', '.pickle', '.txt', '.zip']) or '.' not in file:
#                     continue
#                 file_path = os.path.join(root, file)
#                 if os.path.isfile(file_path):
#                     all_files.append((file, os.path.getsize(file_path), root))
#     return all_files
...
#     # Gather all files from all directories
#     all_files = gather_all_files(sample_dir)

#     # Sort all files by the number of files in their directory, then by file size
#     all_files.sort(key=lambda x: (count_files_in_directory(x[2]), x[1]))

#     tasks = []

#     # Prepare the tasks
#     for file, size, root in all_files:
#         save_dir = os.path.join(save_dir_base, os.path.basename(root))
#         create_directory(save_dir + '/')
#         tasks.append((file, root, save_dir + '/', 1))
...
#------------------------------------------------------
def count_files_in_directory(directory):
    # 디렉토리 내의 파일 수를 반환 (서브 디렉토리는 제외)
    return sum([len(files) for _, _, files in os.walk(directory)])

def list_files_by_size(directory):
    # 디렉토리 내 파일을 크기 순으로 정렬하여 반환
    files_with_sizes = []
    for root, dirs, files in os.walk(directory):
        for file in files:
            file_path = os.path.join(root, file)
            if os.path.isfile(file_path):
                files_with_sizes.append((file, os.path.getsize(file_path), root))
    # 파일 크기 순으로 정렬 (작은 순서대로)
    files_with_sizes.sort(key=lambda x: x[1])
    return files_with_sizes

def process_directory(sample_dir, save_dir_base, number_of_nop=1):
    # 전체 디렉토리와 파일 개수를 가져와 파일 개수 기준으로 정렬
    directories = [(root, count_files_in_directory(root)) for root, dirs, files in os.walk(sample_dir) if len(files) > 0 and 'ok' not in root.split(os.sep)]
    directories.sort(key=lambda x: x[1])  # 파일 개수가 적은 디렉토리부터 정렬

    for root, file_count in directories:
        if file_count >= 20:  # 파일이 너무 적은 디렉토리는 건너뛰기
            continue
        
        # 저장할 디렉토리 경로 생성
        fam = os.path.basename(root)
        save_dir = os.path.join(save_dir_base, fam)
        create_directory(save_dir)

        # 해당 디렉토리 내의 파일을 크기 순으로 정렬
        samples = list_files_by_size(root)

        for sample, size, sample_root in samples:
            # 무시해야 할 확장자나 조건들에 해당하는 파일 건너뛰기
            if any(ext in sample for ext in ['.ipynb', '.pickle', '.txt', '.zip']) or '.' not in sample:
                continue
            ...
            file_path = os.path.join(sample_root, sample)
...
if __name__ == "__main__":
    ...
    # 디렉토리 내 파일 개수가 적은 디렉토리부터, 각 디렉토리 내 파일 크기가 작은 파일부터 처리
    process_directory(sample_dir, save_dir_base, number_of_nop=1)
