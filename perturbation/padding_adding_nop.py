...
def modify_headers(file_path, new_text, padding_size_dict, fin=None):
    ...
    for section in pe.sections:
        ...
        if section_name == list(padding_size_dict.keys())[target_section_len]:
            #print("modify_headers- ", section_name)
            text_section = section
            #print("new_text+_len : ",len(new_text))
            ...
            if len(padding_size_dict.keys()) == target_section_len:
                break
...
def modify_rdata(save_dir, modified_address, padding_size_dict, fin = None):
    # PE 파일 로드
    file_path = save_dir
    pe = pefile.PE(file_path)
    print("modify_rdata : hihi")

    # target_section_len을 위한 초기값 설정
    target_section_len = 0

    # .rdata 섹션 찾기
    for section in pe.sections:
        # SECTION_CHARACTERISTICS를 기반으로 .rdata 또는 .data 섹션을 찾음
        if section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_CNT_INITIALIZED_DATA'] and \
           ...
           section.Characteristics & pefile.SECTION_CHARACTERISTICS['IMAGE_SCN_MEM_WRITE']:
            ...
            rdata_start = section.VirtualAddress#+image_base
            rdata_end = rdata_start + section.SizeOfRawData
            
            section_size = section.SizeOfRawData
            section_start = section.PointerToRawData
            print("rd : ",hex(rdata_start), hex(rdata_end), hex(section_start))

            data = bytearray(pe.get_memory_mapped_image()[rdata_start:rdata_start + section_size])

            # 절대 주소 필터링을 위한 범위 설정
            
            # padding_size_dict에서 섹션 이름 가져오기
            section_name_to_find = list(padding_size_dict.keys())[target_section_len]

            # section_name_to_find와 일치하는 섹션 찾기
            text_section = next(
                section for section in pe.sections
                if section.Name.decode().strip('\x00').lower() == section_name_to_find
            )
            print("rdata - ",text_section)
...
@lru_cache(maxsize=None)
def count_instructions(first_address, second_address, section_data, bit, number_of_nop, jump_dict, nop_cnt, increase_instr, pe_data, text_section, image_base, virtual_address, last_nop_address):
    ...
    for instr in decoder:
        ...
        if start_address <= instr.ip <= end_address:
            ...
            if (last_nop_address !=0) and (instr.ip >= last_nop_address):
                number_of_nop = 0
            ...
            if instr.ip == end_address:
                ...
                if not should_add_nop(str(instr)):
                    if len(instr)==2 and 'REL' in code_to_string(instr.code):
                        ...
                        anc = count_instructions_between_addresses(instr.ip, int(address,16), section_data, bit, (image_base+virtual_address), number_of_nop, last_nop_address)

                        total_increase = calculate_instruction_length_increase(instr.ip, int(address,16), section_data, bit, dict_to_frozenset(jump_dict), dict_to_frozenset(increase_address), number_of_nop, pe_data, text_section, image_base, virtual_address, last_nop_address)
...
@lru_cache(maxsize=None)
def count_instructions_between_addresses(first_address, second_address, section_data, bit, addr, number_of_nop, last_nop_address):
    #decoder = Decoder(bit, section_data, ip=addr)
    decoder = decode_section_data(bit, section_data, addr)
...
@lru_cache(maxsize=None)
def calculate_instruction_length_increase(first_address, second_address, section_data, bit, jump_dict, increase_address, number_of_nop, pe_data, text_section, image_base, virtual_address, last_nop_address):
    addr = image_base+virtual_address
    #decoder = Decoder(bit, section_data, ip=addr)
    decoder = decode_section_data(bit, section_data, addr)
    jump_dict = dict(jump_dict)
            ...
            if instr.ip == end_address:
                ...
                if not should_add_nop(str(instr)):
                    ...
                    if len(instr)==2 and 'REL' in code_to_string(instr.code):
                        ...
                        anc = count_instructions_between_addresses(instr.ip, int(address,16), section_data, bit, addr, number_of_nop, last_nop_address)
...
def make_new_text(file_path ,number_of_nop, padding_size_dict):
    print("sibal")
    global old_rawPointer
    global old_nextPointer
    ...
    for section in pe.sections:
        ...
        if section_name not in list(padding_size_dict.keys()):
            print("  pass")
            continue
        ...
        if section_name == list(padding_size_dict.keys())[target_section_len]:
            ...
            print("match :",section.Name.strip(b'\x00'), ' | ', len(new_text_list), ' | ',target_section_len)
            ...
            for instr in tqdm(decoder):                 
                ...
                if abs(padding_size_dict[section_name] - (nop_cnt+increase_instr)) <= 5:
                    number_of_nop = 0    
                    
                    if last_nop_address == 0:
                        last_nop_address = instr.ip
                ...
                if 'REL' not in code_to_string(instr.code): # 절대주소
                    ...
                    if text_start <= value <= text_end:
                        ...
                        adding_nop_cnt, increace_instruction, increase_address = count_instructions(instr.ip, value, section_text, bit, number_of_nop, dict_to_frozenset(jump_dict), nop_cnt, increase_instr, pe_data, text_section, image_base, virtual_address, last_nop_address) 
                        ...
                        #print(target_address, hex(target_address))
                        
                        if isinstance(target_address, str):
                            target_address = int(target_address, 0)  # 0은 자동으로 10진수나 16진수를 처리
                ...
                if 'REL' in code_to_string(instr.code):
                    ...
                    if instr.ip < address:
                        ...
                        adding_nop_cnt, increace_instruction, increase_address = count_instructions(instr.ip, address, section_text, bit, number_of_nop, dict_to_frozenset(jump_dict), nop_cnt, increase_instr, pe_data, text_section, image_base, virtual_address, last_nop_address) 

                        i_cnt=0
                        i_cnt = sum(increase_address.values())

                        target_address = len(instr) + present_address + (int_operand + adding_nop_cnt + (i_cnt))
                        #print("   Tlqkf target_address : ",hex(target_address))
            ...
            #target_section_len+=1
            #print("seul ma : ",len(new_text_list), section_name, ' | ',target_section_len)
...
def valid_address_check(file_path, save_dir, caller_callee_dict, checking_target_address, modified_address, number_of_nop, padding_size_dict):
    ...
    for section in pe.sections:
        section_name = section.Name.decode().strip('\x00').lower()
        if section_name == list(padding_size_dict.keys())[target_section_len].lower():
            text_section = section
            ...
            for target, destination in modifying_address.items():
                ...
                print(address_type, address, instruction, hexdump, type(hexdump), size)
    ...
    #new_text = modify_headers(file_path, binary_data_list, padding_size_dict, fin=1)
    #save_dir = modify_section(file_path, new_text, save_dir, str(number_of_nop), fin=1)
    modify_rdata(save_dir, modified_address, padding_size_dict, fin=1)
