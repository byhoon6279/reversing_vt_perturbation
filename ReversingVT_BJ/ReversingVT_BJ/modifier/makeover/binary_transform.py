...
def randomize(input_file, n_randomize=10):
  ...
  for i_r in range(n_randomize):
    ...
    for f in filter(lambda x: x.level != -1, functions.values()):
...
def parse_arguments():
    ...
    parser.add_argument('--dir', type=str, help="Directory containing .exe files to randomize.")
    parser.add_argument('--disp_budget', type=float, help='File size increase due to displacement (ratio)')
    parser.add_argument('--pad_val', type=str, default=256, help='Padding value (default: 256)')
    parser.add_argument('--iters', type=int, default=1, help='Number of randomizing iterations')
...
if __name__=="__main__":
    ...
    binary_paths = args.pe if args.pe else []
    ...
    if args.dir:
        ...
        exe_files = [os.path.join(args.dir, f) for f in os.listdir(args.dir) if f.endswith(".exe")]
        if not exe_files:
            print(f"Error: No .exe files found in directory '{args.dir}'")
            sys.exit(1)

        binary_paths.extend(exe_files)  # 기존 파일 목록에 추가
    ...
    # ✅ 모든 .exe 파일에 대해 randomize() 실행
    with open("randomization.log", "a", encoding="utf-8", errors="replace") as log_file:
        for bin_path in binary_paths:
            safe_bin_path = bin_path.encode(errors="replace").decode()
            print(f'====================')
            print(f'Randomizing "{safe_bin_path}"...')
            log_file.write(f'Randomizing "{safe_bin_path}"...\n')
