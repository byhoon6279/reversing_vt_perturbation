...
def patch(pe_file, disp_state, diffs):
    ...
    for ea, orig, new in diffs:
        if (disp_state is None) or ea < disp_state.ropf_start:  # non-displaced instruction
            if ea < base:
                # ✅ `new`가 str이면 `ord()`로 변환 후 bytes로 변환
                if isinstance(new, str):
                    new = ord(new)
                if not pe_file.set_bytes_at_offset(ea, bytes([new])):
                    print("error setting bytes")
            else:
                curr = pe_file.get_data(ea - base, 1)

                # ✅ bytes → int 변환하여 비교
                if curr is not None and isinstance(curr, bytes) and len(curr) == 1:
                    curr = curr[0]
                if orig is not None and isinstance(orig, bytes) and len(orig) == 1:
                    orig = orig[0]

                if orig is not None and curr != orig:
                    print("error in patching", hex(ea), ":", curr, "!=", orig)

                # ✅ `new`가 str이면 `ord()`로 변환 후 bytes로 변환
                if isinstance(new, str):
                    new = ord(new)
                if not pe_file.set_bytes_at_rva(ea - base, bytes([new])):
                    print("error setting bytes")
