...
def ef_altering_nop(n_bytes):
    ...
    if n_bytes>=4:
        ...
        v = struct.pack('<B', random.randint(1, 255))  # ✅ `bytes` 변환
...
def reg_altering_nop(n_bytes, protected):
    ...
    if n_bytes >= 5:
        if protected['eax']:
            v = struct.pack('<I', random.randint(0,2**31-1))  # ✅ `bytes` 타입 유지
            options.extend([b'\x05' + v] * UF * UF)  # ✅ 문자열이 아니라 `bytes`로 연결
            options.extend([b'\x15' + v] * UF * UF)
            options.extend([b'\x2D' + v] * UF * UF)
            options.extend([b'\x1D' + v] * UF * UF)
            options.extend([b'\x0D' + v] * UF * UF)
            options.extend([b'\x25' + v] * UF * UF)
            options.extend([b'\x35' + v] * UF * UF)

    if n_bytes >= 6:
        if protected['ebx']:
            v = struct.pack('<I', random.randint(0, 2**31 - 1))
            options.extend([b'\x81\xc3' + v] * UF * UF)
            options.extend([b'\x81\xd3' + v] * UF * UF)
            options.extend([b'\x81\xeb' + v] * UF * UF)
            options.extend([b'\x81\xdb' + v] * UF * UF)
            options.extend([b'\x81\xcb' + v] * UF * UF)
            options.extend([b'\x81\xe3' + v] * UF * UF)
            options.extend([b'\x81\xf3' + v] * UF * UF)
        if protected['ecx']:
            v = struct.pack('<I', random.randint(0,2**31-1))
            options.extend([b'\x81\xc1'+v]*UF*UF) # add ecx, v
            options.extend([b'\x81\xd1'+v]*UF*UF) # adc ecx, v
            options.extend([b'\x81\xe9'+v]*UF*UF) # sub ecx, v
            options.extend([b'\x81\xd9'+v]*UF*UF) # sbb ecx, v
            options.extend([b'\x81\xc9'+v]*UF*UF) # or ecx, v
            options.extend([b'\x81\xe1'+v]*UF*UF) # and ecx, v
            options.extend([b'\x81\xf1'+v]*UF*UF) # xor ecx, v
        if protected['edx']:
            v = struct.pack('<I', random.randint(0,2**31-1))
            options.extend([b'\x81\xc2'+v]*UF*UF) # add edx, v
            options.extend([b'\x81\xd2'+v]*UF*UF) # adc edx, v
            options.extend([b'\x81\xea'+v]*UF*UF) # sub edx, v
            options.extend([b'\x81\xda'+v]*UF*UF) # sbb edx, v
            options.extend([b'\x81\xca'+v]*UF*UF) # or edx, v
            options.extend([b'\x81\xe2'+v]*UF*UF) # and edx, v
            options.extend([b'\x81\xf2'+v]*UF*UF) # xor edx, v
    ...
    if isinstance(choice, str):
        choice = choice.encode('latin1')

    # ✅ unconstrained idxs 설정
    uidxs = []
    if len(choice) == 2:
        if choice[0:1] in [b'\xB0', b'\xB4', b'\x04', b'\x14', b'\x2C', b'\x1C',
                            b'\x0C', b'\x24', b'\x34', b'\xB3', b'\xB7', b'\xB1',
                            b'\xB5', b'\xB2', b'\xB6']:
            uidxs = [1]
    if len(choice) == 5:
        if choice[0:1] in [b'\x05', b'\x15', b'\x2D', b'\x1D', b'\x0D', b'\x25', b'\x35']:
            uidxs = [1, 2, 3, 4]
    if len(choice) == 6:
        if choice[0:1] == b'\x81':
            uidxs = [2, 3, 4, 5]
...
def get_semantic_nop(n_bytes, protected=None, start_idx=None, \
    ...
    if nop_type == 'atomic_nop':
        ...
        pre = atomic_nop(n_atomic)  # ✅ 이미 `bytes` 타입이면 변환하지 않음
        if isinstance(pre, str):
            pre = pre.encode('latin1')  # `str`이면 `bytes` 변환

        suf, unconstrained_idxs = get_semantic_nop(n_bytes - n_atomic, protected, start_idx + len(pre), True)

        if isinstance(suf, str):  # ✅ `suf`도 `bytes`로 변환
            suf = suf.encode('latin1')

        res = pre + suf  # ✅ 이제 `bytes + bytes`라 오류 없음!
        ...
        tmp = protected['ecx']
        protected['ecx'] = True
        mid, unconstrained_idxs = get_semantic_nop(n_bytes-2, protected, start_idx+1, True)

        # ✅ 모든 값을 bytes 타입으로 변환
        if isinstance(mid, str):
            mid = mid.encode('latin1')

        res = b'\x51' + mid + b'\x59'  # ✅ bytes + bytes + bytes
        protected['ecx'] = tmp
        ...
        pre, suf = combo_nop()
        pre = pre.encode('latin1') if isinstance(pre, str) else pre
        ...
        raise ValueError(f'Unknown nop type: {nop_type}')
