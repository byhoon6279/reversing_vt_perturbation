...
nop = pydasm.get_instruction(b'\x90', pydasm.MODE_32)

# try:
#     # 바이너리 데이터와 모드를 정확히 전달
#     nop = pydasm.get_instruction(b'\x90', pydasm.MODE_32)
#     print(nop)
#     time.sleep(20)
...
class Instruction(object):
  ...
  def __init__(self, ea, bytes, spd):
    ...
    if inst == None:
        print("IGNORE:", hex(ea), ''.join(('\\x%02x' % b for b in bytes)))
        inst = nop
...
#    self.bytes = str(self.cbytes)
  ...
  def _get_use_def(self, inst):
    ...
    if not inst.ptr.checked:  # and self.mnem not in ("bswap", "fimull", "cmpsb"):
        print(("NOT TRACKING:", ''.join(['\\x%02x' % b for b in self.bytes]), self.disas))
        return
    ...
    if (len(self.mnem)>=5 and self.mnem.startswith(b'cmov')):
      #self.DEF.add(REGS[inst.op1.reg])
  ...
  def __str__(self):
    ...
    return "%3d: 0x%08X %s \t(%s)" % (
        self.pos, self.addr, self.disas,
        ''.join(['%02x' % self.bytes[i] for i in range(len(self.bytes))])
    )
    #for reg in REGS[:8]:
