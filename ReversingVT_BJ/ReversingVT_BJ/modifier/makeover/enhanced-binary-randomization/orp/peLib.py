   ...
   # Adjust IMAGE_FILE_HEADER
    ...
    def _update_optional_header(self, SizeOfRawData, VirtualAddress):
        ...
        NT_HEADERS - IMAGE_OPTIONAL_HEADER elements to update (36 fields, 96B in total)
            ...
            Checksum
        """
         # 🔹 정수 변환 추가 (float → int 변환)
        SizeOfRawData = int(SizeOfRawData)
        VirtualAddress = int(VirtualAddress)

        # 🔹 정수 나눗셈 연산 수정 (float 방지)
        def __multiple_round(n, r):
            return (n + (r - 1)) // r * r  # ✅ `/` → `//` 변경 (정수 나눗셈)
        ...
        self.peinfo.setSizeOfCode(int(newSizeOfCode + SizeOfRawData))
        self.peinfo.setSizeOfInitializedData(int(newSizeOfInitializedData))
        self.peinfo.setSizeOfUninitializedData(int(newSizeOfUninitializedData))

        # 🔹 Checksum 오류 방지 (모든 값 int 변환)
        self.pe.OPTIONAL_HEADER.SizeOfCode = int(self.pe.OPTIONAL_HEADER.SizeOfCode)
        self.pe.OPTIONAL_HEADER.SizeOfImage = int(self.pe.OPTIONAL_HEADER.SizeOfImage)
        self.pe.OPTIONAL_HEADER.SizeOfInitializedData = int(self.pe.OPTIONAL_HEADER.SizeOfInitializedData)
        self.pe.OPTIONAL_HEADER.SizeOfUninitializedData = int(self.pe.OPTIONAL_HEADER.SizeOfUninitializedData)
        self.pe.OPTIONAL_HEADER.ImageBase = int(self.pe.OPTIONAL_HEADER.ImageBase)
        self.pe.OPTIONAL_HEADER.AddressOfEntryPoint = int(self.pe.OPTIONAL_HEADER.AddressOfEntryPoint)
    ...
    def _adjust_reloc(self, moving_regions, reloc_entries):
        ...
        for (data, rva, type) in reloc_entries:
            if type == 0:
                continue
            base = rva & 0xFFFFF000
            if base not in list(reloc_blocks.keys()):
        ...
        for b_base in sorted(reloc_blocks.keys()):
            for entry in reloc_blocks[b_base]:
                ...
                if mov_start == 0:
                    if b_base not in list(new_reloc_blocks.keys()):
                ...
                # Case that the reloc addr should be updated due to displaced regions in .text section
                    ...
                    if new_base not in list(new_reloc_blocks.keys()):
        ...
        for b_base in sorted(new_reloc_blocks.keys()):
            new_reloc_data += struct.pack('<I', b_base)  # ✅ bytes + bytes
            ...
            for entry in sorted(new_reloc_blocks[b_base]):
                new_reloc_data += struct.pack('<H', entry)  # ✅ bytes + bytes

            if b_size % 4 != 0:
                new_reloc_data += struct.pack('<H', 0)  # ✅ bytes + bytes
    ...
    def update_displacement(self, disp_state, DEBUG=False):
        ...
        if ActualCodeSize % FileAlignment != 0: # Padding the code up to FileAlignment
            moving_code += b'\x00' * (FileAlignment - ActualCodeSize % FileAlignment)
        ...
        if ActualCodeSize % FileAlignment == 0:
            ...
            SizeOfRawData += ((VirtualSize // FileAlignment) * FileAlignment) + FileAlignment  # ✅ 정수 연산
        ...
        self.pe.set_dword_at_offset(offsetToNewSection + 0x10, int(SizeOfRawData))
