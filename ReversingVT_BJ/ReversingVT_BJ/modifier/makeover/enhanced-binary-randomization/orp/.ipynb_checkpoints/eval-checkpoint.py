...
def get_changed_bytes(functions):
  ...
  for f in [x for x in iter(list(functions.values())) if x.level != -1]:
...
def eval_coverage(input_file, dump_eval_data=False):
  ...
  def _check_gadget(g, changed, ins_hit):
    ...
    for i_start, i_end in zip(g.addrs, g.addrs[1:]+[g.end]):
      if any(b in changed for b in range(i_start, i_end)):
        ins_hit.add(i_start)
        broke = True
  ...
  for g in gadgets:
    ...
    if g.overlap and _check_gadget(g, reorder_b, ins_hit):
      ...
      for ins in (f.code[a] for a in range(g.start, g.end) if a in f.code):
  ...
  if len(gadgets) > 0:
    ...
    for k, v in list(ins_hit_data.items()):
...
def get_func_entropy(f, live_regs, swaps, preservs, avail_regs):
  ...
  entropy += np.log(reduce(lambda x, y: x | y, (i.implicit for i in f.instrs if not i.f_exit), set()) + 1) / np.log(2)
...
def eval_exploit(input_file, dump_eval_data=False):
  ...
  for func_ea, gadgets in list(func_gads.items()):
