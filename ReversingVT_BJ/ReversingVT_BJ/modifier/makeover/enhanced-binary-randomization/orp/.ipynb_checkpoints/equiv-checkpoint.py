...
def check_equiv(ins):
    ...
    opcode = ins.bytes[ins.opc_off]
    modrm = ins.bytes[ins.modrm_off]
...
#          tmp = struct.unpack('b',  chr(ins.cbytes[-1]))[0]
          tmp = struct.unpack('b', bytes([ins.cbytes[-1]]))[0]
...
#          tmp = struct.unpack('i',  str(ins.cbytes[-4:]))[0]
          tmp = struct.unpack('i', bytes(ins.cbytes[-4:]))[0]
...
if __name__ == "__main__":
  ...
  for head in idautils.Heads(start_ea, end_ea):
    ...
    if check_equiv(ins):
      eq_ins = insn.Instruction(head, bytes(ins.cbytes), 0)
      print(ins.disas, "(%s) ->" % ins.bytes.encode("hex"), end=' ')
