...
def run(cmd, timeout):
  ...
  output = ''.join(proc.communicate()[0].splitlines())
...
def get_addr_range(addrs, addr):
    ...
    Return the range of either functions/blocks/codes with a binary search, O(log N)
    """
    starts, ends = zip(*addrs)  # ⬅️ `zip()`을 한 번만 사용하도록 수정
...
def draw_graph(g, filename="cfg.eps"):
  """Draws the given graph (which should be a pygraph instance)."""

  from pygraph.readwrite.dot import write as write_graph

  input_lines = write_graph(g).split('\n')
  input_lines.insert(1, 'node [shape=box, style=rounded, fontname=Courier]')
  input_data = '\n'.join(input_lines).encode("utf-8")  # ⬅️ 문자열을 바이트로 변환

  with open(filename, 'wb') as f:  # ⬅️ 바이너리 모드로 파일 열기
    proc = Popen(['dot', '-q', '-Teps'], stdin=PIPE, stdout=f, stderr=PIPE)
    _, stderr = proc.communicate(input_data)

  if stderr:
    print("dot failed:", stderr.decode("utf-8"))  # ⬅️ 바이트 데이터를 문자열로 변환
...
#   live_regs_in_pos = {}
#   for reg, lifetime in list(live_subsets.items()):
...
#       ret_str += '<TD '
#       if reg in list(live_reg_types.keys()):
...
def draw_CFG(func, live_subsets):
    ...
    # Find all live registers at each position
    live_regs_in_pos = {}
    for reg, lifetime in live_subsets.items():  # Python 3에서는 .items() 사용 가능
        for subset in lifetime.subsets:
            for instr in subset.instr_set:
                box_type = BOX_LIVE
                # Highlight instructions that define a register
                if reg in instr.OUT and reg not in instr.IN:
                    box_type = BOX_DEF
                live_regs_in_pos.setdefault(instr.pos, {}).update({reg: box_type})
    ...
    def get_liveregs_str(pos):
        ...
        ret_str = ''
        live_reg_types = live_regs_in_pos.get(pos, {})

        # Draw registers in this particular order, ignore esp for now
        for reg in ['eax', 'ebx', 'ecx', 'edx', 'esi', 'edi', 'ebp']:
            ret_str += '<TD '
            if reg in live_reg_types:
                ret_str += boxes[live_reg_types[reg]] % (reg_colors[reg], reg)
            else:
                ret_str += boxes[BOX_NONE] % reg
            ret_str += '</FONT></TD>'
    ...
    for ins in func.instrs:
        for suc in ins.succ:
            dotfile_lines.append(f'{ins.pos} -> {func.code[suc].pos};')

    dotfile_lines.append('}')
    
    dot_input = '\n'.join(dotfile_lines).encode("utf-8")  # ⬅️ 바이트로 변환 필요

    # Generate graph
    with open('cfg.eps', 'wb') as f:  # ⬅️ 바이너리 모드로 파일 열기
        proc = subprocess.Popen(['dot', '-q', '-Teps'], stdin=subprocess.PIPE,
                                stdout=f, stderr=subprocess.PIPE)
        _, stderr = proc.communicate(dot_input)  # ⬅️ 바이트 데이터를 전달

    if stderr:
        print("dot failed:", stderr.decode("utf-8"))  # ⬅️ 바이트 → 문자열 변환 필요
