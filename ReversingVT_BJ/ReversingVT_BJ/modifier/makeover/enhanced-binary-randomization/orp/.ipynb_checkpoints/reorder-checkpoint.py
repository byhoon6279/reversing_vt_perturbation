...
def ReorderGraph(dag):
  ...
  ordering = []
  edge_srcs, edge_dsts = list(zip(*dag.edges()))    # unzip list of edge tuples
...
#   roots.update(set(dag.nodes()) - set(itertools.chain(*dag.edges())))
  roots.update(set(dag.nodes()) - set(itertools.chain(*list(dag.edges()))))  # ✅ 리스트 변환 추가    
...
def causes_reloc_diff(rinstrs, pe, relocations):
  ...
  for rins in [x for x in rinstrs if x.inst_len >= 5]:
  ...
  if use_caching and _xxxlastfile == id(pe):
    ...
    if pe.OPTIONAL_HEADER.DATA_DIRECTORY[5].Size > 0 and \
      ...
      for base_reloc in pe.DIRECTORY_ENTRY_BASERELOC:
        for reloc in [x for x in base_reloc.entries if x.type == 3]:  #HIGHLOW
...
def can_reorder(f):
  ...
  for block in f.blocks:
    ...
    edge_srcs, edge_dsts = list(zip(*dag.edges()))
    roots = set(edge_srcs) - set(edge_dsts)
    #roots.update(set(dag.nodes()) - set(itertools.chain(*dag.edges()))) 
    roots.update(set(dag.nodes()) - set(itertools.chain(*list(dag.edges()))))  # ✅ 리스트 변환 추가
    roots = deque(roots)
