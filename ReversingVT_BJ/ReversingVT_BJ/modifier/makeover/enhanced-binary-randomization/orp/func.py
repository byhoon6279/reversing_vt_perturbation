...
class Function(object):
  ...
  def _get_instrs_graph(self):
    #initialize graph
    igraph = digraph()
    igraph.add_nodes(iter(list(self.code.values())))
  ...
  def analyze_registers(self, functions):
    ...
    if not self.check_SEH_preservs(functions):
      ...
      for push in pushes:
        ...
        reg_pops = [x for x in pops if reg in x.DEF]
  ...
  def update_returns(self, set_default=False):
    for ins in [x for x in self.instrs if x.f_exit]:
      if set_default:
  ...
  def update_calls(self):
    ...
    for ins in [x for x in self.instrs if x.mnem == "call"]:
...
def classify_functions(functions):
  ...
  while len(processed) < len(functions):
    ...
    for func in [x for x in iter(list(functions.values())) if x.level == -2]:
...
def analyze_functions(functions, levels):
  ...
  for func in [x for x in iter(list(functions.values())) if x.level == -1]:
  ...
  for l in range(levels):
    ...
    for func in [x for x in iter(list(functions.values())) if x.level == l]:
  ...
  calls = updated = 0
  for func in [x for x in iter(list(functions.values())) if hasattr(x, "instrs")]:
