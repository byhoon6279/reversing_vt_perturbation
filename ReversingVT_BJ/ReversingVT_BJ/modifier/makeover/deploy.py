...
def main(args):
    ...
    if args.env == "cpu":
        ...
        version_output = subprocess.check_output(["sudo", "docker", "--version"]).decode('utf-8')
    ...
    subprocess.check_call(["sudo", "docker", "build", "--tag", tag_name] + build_args + ["."])
    ...
    subprocess.call(["sudo", "docker", "run", "--rm", "-it", "--privileged", "-u", "{}:{}".format(os.getuid(), os.getgid()), "--name", "binary_transform"] +
                    user_args +
                    gpu_args +
                    volume_mappings +
                    runtime_args +
                    ...
                    [ "-u", "root",  # 추가된 부분
                    "--network", "host",
                    "-v", "/opt/idapro-8.3:/opt/idapro-8.3",
                    "-e", "PATH=/opt/idapro-8.3:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
                    "-e", "LD_LIBRARY_PATH=/opt/idapro-8.3:/lib/x86_64-linux-gnu:$LD_LIBRARY_PATH",
                    ] +
                    ["{}:latest".format(tag_name), "/bin/bash", "-c", "apt-get update && apt-get install -y python3.8 python3.8-dev python3.8-distutils libsecret-1-0 && /bin/bash"])
...
#         "-e", "PATH=/opt/idapro-8.3:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin",
#         "-e", "LD_LIBRARY_PATH=/opt/idapro-8.3:/lib/x86_64-linux-gnu:$LD_LIBRARY_PATH",
#         "{}:latest".format(tag_name), 
#         "/bin/bash", "-c",
#         "apt-get update && apt-get install -y python3.8 python3.8-dev python3.8-distutils libsecret-1-0 && exec /bin/bash"
#     ])

    
#     subprocess.call(["sudo", "docker", "run", "--rm", "-it", "--name", "binary_transform"] +
#                 user_args +
#                 gpu_args +
#                 volume_mappings +
#                 runtime_args +
