...
if __name__ == '__main__':
    #idaapi.autoWait()
    idaapi.auto_wait()
    #pe_filename = GetInputFile()
    pe_filename = idc.get_input_file_path()
    print("1 : ",pe_filename)
    #asdf()
    #pe_filename = pe_filename.split(".")[0]
    pe_filename = pe_filename.split("/")[-1]
    print('pe_filename 2 : ',pe_filename)
    ...
    saved_path = os.path.join(base_path, config.Acfg.saved_path)
    ...
    print("gdl_path : ",gdl_path)
    print('pe_filename : ',pe_filename)
    #pe_filename = pe_filename.replace('.exe','.gdl')
    gdl_filename = os.path.join(gdl_path, pe_filename)
    gdl_filename = gdl_filename.replace('.exe','')
    print("gdl_filename : ",gdl_filename)
    #time.sleep(5)
